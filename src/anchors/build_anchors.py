...
def _config_numbers(path: Path, id_col: str) -> dict:
    """Numeric values from a two-column configuration register, keyed on its id."""
    out = {}
    with open(path, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            try:
                out[row[id_col]] = float(row["value"])
            except (ValueError, KeyError):
                continue
    return out


# Treasury policy and the credit agreement are configuration shared with the Phase 2
# generator, so a policy cannot be changed in one layer and not the other.
TREASURY = _config_numbers(ROOT / "config" / "debt" / "treasury_policy.csv", "policy_id")
CREDIT_AGREEMENT = _config_numbers(
    ROOT / "config" / "debt" / "credit_agreement_terms.csv", "term_id")
...
# Interest is built from its components rather than asserted as a blended rate, so every
# charge can be traced to a clause of the credit agreement (Phase 2.2, ADR-0018).
#
#   base rate            the average SOFR fixing for the year
#   term loan margin     CA-003, 425bps
#   revolver margin      CA-033, 425bps -- the facility ranks pari passu with the term loan
#   hedging              CA-008, USD 100m notional swapped at 3.00% fixed, ~50% of the
#                        term loan, so half the term loan pays 3.00% + 425bps
#   commitment fee       CA-007, 50bps on the average daily UNDRAWN commitment
#
# The base rates below reproduce the previously approved blended term-loan rates exactly;
# they are the SOFR path those rates always implied, now made explicit.
SOFR_BASE = series(FY2023A=0.0470, FY2024A=0.0520, FY2025A=0.0410, FY2026B=0.0310, FY2026F=0.0340)
TLB_MARGIN = CREDIT_AGREEMENT["CA-003"]
RCF_MARGIN = CREDIT_AGREEMENT["CA-033"]
SWAP_FIXED_RATE = CREDIT_AGREEMENT["CA-034"]
SWAP_NOTIONAL = CREDIT_AGREEMENT["CA-035"]
LEASE_RATE = series(FY2023A=0.0650, FY2024A=0.0650, FY2025A=0.0650, FY2026B=0.0640, FY2026F=0.0640)
RCF_COMMITMENT = CREDIT_AGREEMENT["CA-006"]
RCF_COMMITMENT_FEE = CREDIT_AGREEMENT["CA-007"]

# Blended term-loan rate: the hedged portion pays the swap rate plus the margin, the rest
# floats.  The revolver is unhedged and floats in full.
TLB_RATE = {k: 0.5 * (SWAP_FIXED_RATE + TLB_MARGIN) + 0.5 * (SOFR_BASE[k] + TLB_MARGIN)
            for k in COLS}
RCF_RATE = {k: SOFR_BASE[k] + RCF_MARGIN for k in COLS}
...
# Average DAILY drawn balance on the revolving facility.  Interest and the commitment fee
# accrue on the daily position, not on a month-end balance, so this is the only figure that
# can price the facility.
#
# For the actual years it is DERIVED from the generated utilisation model
# (data/reference/revolver_utilisation.csv, opening + drawings - repayments = closing, with
# drawings taken on the borrowing-notice date and repayments on the collections sweep date)
# and is regenerated by tools/derive_anchor_inputs.py.  Phase 2.1 carried a treasury
# forecast of 15.0 / 12.0 / 5.0 which the facility's own roll-forward contradicted -- the
# balance opened FY2025 at 23.8 and closed at 16.3, and an average of 5.0 was not reachable
# from those endpoints at any credible liquidity profile.  See ADR-0018.
#
...
NCI_INCOME = series(FY2023A=-0.197389, FY2024A=-0.218966, FY2025A=-0.265949, FY2026B=-0.179968, FY2026F=-0.179968)
...
# CTA movement, DERIVED (Phase 2.2, ADR-0017).  Phase 1 carried a provisional economic
# estimate of 3.500 / -5.300 / 8.800.  The independently generated entity ledgers prove a
# different value, so the estimate is superseded by the arithmetic:
#
#   CTA(group) = CTA(layer 1, from source net assets and the approved FX policy)
...
#
# The layer-1 term is computed from the source ledgers alone by
