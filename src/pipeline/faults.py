...
EXPECTED = {
    ...
    "F02": (None, "an entity-pair intercompany mismatch is invisible until the elimination "
                  "engine matches the pair, which is Phase 4. Both trial balances still "
                  "close and every Phase 3 control is correct to pass"),
...
def detect() -> list[dict]:
    ...
    with tempfile.TemporaryDirectory(prefix="p3-faults-") as tmp:
        for fault in catalogue:
            ...
            if family is None:
                status = "DEFERRED_TO_LATER_PHASE" if not hit else "ACCIDENTAL_DETECTION"
            elif in_family:
                status = "DETECTED"
            elif hit:
            ...
            out.append(dict(
                ...
                phase3_control_family=family or "n/a (later phase)",
...
def main() -> int:
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        w.writeheader()
        w.writerows(results)
    good = {"DETECTED", "DEFERRED_TO_LATER_PHASE"}
    for row in results:
