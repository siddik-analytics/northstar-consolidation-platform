...
| FX-P06 | **CTA — movement** | Derived | Computed for the period, and tested against the source-layer expectation in `data/reference/cta_expectation.csv` (`CTL-FX-12`) |
...
The obligation runs back into the source layer as well, which Phase 2.2 established the hard
way. A frozen balance is only frozen if the **entity's own ledger** carries it as a fixed
amount in its own currency: share capital does not move because a spot rate moved. A generator
that re-pins contributed capital to a closing-rate USD target each year produces ledgers in
which no translation adjustment can arise at all, and the difference has to be absorbed
somewhere. `P2-FX-01` tests it at source, entity by entity and month by month.
...
Since Phase 2.2 the recomputation is not merely a formula in a control; it is a **published
dataset produced before the engine exists**. `data/reference/cta_expectation.csv` carries one
row per foreign entity and month — opening net assets in local currency, the prior closing,
closing and average rates, the result, the equity movements, and the adjustment each component
generates. It is derived from the source ledgers and the approved rate file alone and
references no group target, so an engine tested against it is genuinely tested (`CTL-FX-12`).

The expectation is only meaningful because the source ledgers hold contributed capital at
**historical rates in the entity's own currency**. Retranslating capital onto a closing-rate
target — which the Phase 2.1 generator did — suppresses the adjustment entirely and leaves a
residual that has to be absorbed somewhere. It was, in a named equity reserve, until Phase 2.2
removed both the cause and the reserve (ADR-0016 superseded by ADR-0017).
...
### 3.4 Group anchor roll-forward — derived, not estimated

Phase 1 set the CTA as an economic estimate and recorded that it remained *a target rather
than a proof* until a translation engine existed. Phase 2 produced entity ledgers that could
test it, and Phase 2.2 derived it from them
([`config/anchors/anchor_cta_rollforward.csv`](../config/anchors/anchor_cta_rollforward.csv),
ADR-0017):

```
CTA(group) = CTA(layer 1, from source net assets and this policy)
...
|---|---|---|---|---|---|
| CTA — opening | (3.50) | (0.89) | (5.56) | 2.18 | 2.18 |
| CTA — movement, group | 2.61 | (4.67) | 7.74 | — | 1.00 |
| CTA — recycled on disposal | — | — | — | — | — |
| **CTA — closing (group)** | **(0.89)** | **(5.56)** | **2.18** | **2.18** | **3.18** |
| Memo: movement attributable to NCI | 0.07 | (0.02) | 0.06 | — | 0.02 |
| **Total translation movement** | **2.68** | **(4.69)** | **7.80** | **—** | **1.02** |

Superseding the provisional Phase 1 target of 3.50 / (5.30) / 8.80 and a closing balance of
— / (5.30) / 3.50. The revision is quantified in full, including its effect on total equity
and the balance sheet, in [`docs/phases/phase-02-2-report.md`](phases/phase-02-2-report.md).
...
| on working capital and other balances | 0.68 | (1.59) | 3.50 | — | (1.58) |
| **Total** | **2.68** | **(4.69)** | **7.80** | **—** | **1.02** |

The decomposition sums to the total by assertion in the anchor model, and it is what routes
the FX effect correctly through the cash flow statement (§4). Goodwill and acquired
intangibles are the only two lines still carried as estimates, because they exist in no source
ledger; every other component is computed from generated balances.
